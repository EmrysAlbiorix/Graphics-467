...
// Does refraction
int refract(double I[3], double N[3], double eta, double R[3]) {
    double dot = I[0] * N[0] + I[1] * N[1] + I[2] * N[2];
    ...
    R[0] = eta * I[0] - (eta * dot + sqrt(k)) * N[0];
    R[1] = eta * I[1] - (eta * dot + sqrt(k)) * N[1];
    R[2] = eta * I[2] - (eta * dot + sqrt(k)) * N[2];
...
int test01() {
	...
	transparency[num_objects] = 0.8; // 60% transparency
  ...
  while (key != 'q') {
		...
		for(x = -400; x < 400; x++) {
			for(y = -400; y < 400; y++) {
				...
				while (insct == 1 && num_intersections < REF_DEPTH) {
		          ...
		          double R[3];
		          int refracted = refract(I, normal, eta, R);
		...
		if (key == 's') {
			G_save_image_to_file("3d_RayRefract_v1.xwd") ;
			G_rgb(1,1,1) ; G_draw_string("Image saved as 3d_RayRefract_v1.xwd", 580,750) ;
			key = G_wait_key() ;
