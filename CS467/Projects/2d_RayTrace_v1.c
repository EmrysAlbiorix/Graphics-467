...
int test01() {
  ...
  M3d_make_identity(vi);  // OVERRIDE for 2d
  //////////////////////////////////////////////////////////////////////
  ...
  Rsource[2] = 0;
  G_rgb(1, 0, 1);
  G_fill_circle(Rsource[0], Rsource[1], 3);
  G_rgb(1, 0, 1);
  G_line(100, 200, 100, 600);
  ...
  for (ytip = 200; ytip <= 600; ytip++) {
    ...
    M3d_mat_mult_pt(RsourceT, obinv[0], Rsource);
    M3d_mat_mult_pt(RtipT, obinv[0], Rtip);

    // x=RsourceT[0]+(RtipT[0]-RsourceT[0])*t
    // y=b+(b-d)*t
    // y=RsourceT[1]+(RtipT[1]-RsourceT[1])*t
    // 1st equation: (a) RsourceT[0]^2 + (b)
    // 2*RsourceT[0]*(RtipT[0]-RsourceT[0]) + (c) (RtipT[0]-RsourceT[0])^2 2nd
    // equation: (a) RsourceT[1]^2 + (b) 2*RsourceT[1]*(RtipT[1]-RsourceT[1]) +
    // (c) (RtipT[1]-RsourceT[1])^2 A: (RtipT[0]-RsourceT[0])^2 +
    // (RtipT[1]-RsourceT[1])^2 B: 2*RsourceT[0]*(RtipT[0]-RsourceT[0]) +
    // 2*RsourceT[1]*(RtipT[1]-RsourceT[1]) C: [RsourceT[1]^2+RsourceT[0]^2-1]

    double A, B, C;
    double t;
    double intersect[3];  // x and y vector to hold intersect point (xt,yt)
