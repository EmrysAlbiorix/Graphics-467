...
int idA ;
void getColor(double u, double v, double uhi, double vhi, double ulo, double vlo) {
	double e ;
	int d[2], x, y ;
	double width, height ;
	double rgb[3];
		  ...
		  double MOVEMENT[4][4]) 
{
	...
	for (u = ulo ; u <= uhi ; u += DETAIL) {
  	for(v = vlo ; v <= vhi ; v += DETAIL) {
   		point[0] = f(u, v) ;
   		point[1] = g(u, v) ;
   		point[2] = l(u, v) ;
		
			if(MOVEMENT == EARTHobj) {
				getColor(u, v, uhi, vhi, ulo, vlo) ;
			}
...
int main() {
	...
	idA = init_xwd_map_from_file ("../XWDhelp/Change_xwd_styles/earthJ.xwd") ;// returns -1 on error, 1 if ok
	...
	q = 0 ;
	int count = 0 ;
	char filename[100] = "SpaceMovie/SpaceMovie.xwd" ;
	//while(q != 'q') {
	while (q < 195) {
	  	init_zbuffer() ;
  	...
  	//draw SHIP
		...
		plot(disk_x, disk_y, disk_z, 0, 2*M_PI, 0, 2*M_PI, TORUSobj) ;
		plot(torus_x, torus_y, torus_z, 0, 2*M_PI, 0, 2*M_PI, TORUS2obj) ;
		plot(disk_x, disk_y, disk_z, 0, 2*M_PI, 0, 2*M_PI, TORUS2obj) ;
  	...
  	//q = G_wait_key(); 
		...
		sprintf(filename, "SpaceMovie/SpaceMovie%04d.xwd", q) ;
