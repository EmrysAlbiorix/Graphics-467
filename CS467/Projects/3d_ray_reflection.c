...
#define hither 2
#define yon 100
#define REF_DEPTH 2 // reflection depth
...
void check_intersect_ref(double source[3], double tip[3], int num_reflections, int x, int y) {

  double intersect[3], normal[3], reflection[3];
  double source_new[3], tip_new[3];
  int obnum, i, boolean;
  ...
  while(num_reflections < REF_DEPTH && boolean == 1) {
          OB = -1;
	  T = 100000000; 
	  for (obnum = 0; obnum < num_objects; obnum++) {
	  M3d_mat_mult_pt(source_new, obinv[obnum], source);
	  M3d_mat_mult_pt(tip_new, obinv[obnum], tip);

	  i = find_intersection(source_new, tip_new, intersect, obnum);
	  }
      	    
	  if (OB > -1) {
	  // find unit normal in obj space
	  i = find_normal(intersect, normal, obinv[OB], OB);
	  objs_reflected[num_reflections] = OB;

	  // send intersection to obj space 
	  M3d_mat_mult_pt(intersect, obmat[OB], intersect);

	  // light model
	  i = Light_Model(Irgb[OB], eye, intersect, normal, lrgb[OB]);
       
	  // make intersection pt the new source; reflection pt the new tip
	  i = find_reflection(intersect, source, normal, reflection);

	  source[0] = intersect[0] + 0.0001 * reflection[0];
	  source[1] = intersect[1] + 0.0001 * reflection[1];
	  source[2] = intersect[2] + 0.0001 * reflection[2];
	  
	  tip[0]    = intersect[0] + reflection[0];
	  tip[1]    = intersect[1] + reflection[1];
	  tip[2]    = intersect[2] + reflection[2];
	  ...
	  //printf("%d\n", num_reflections);
	  } else {boolean = 0;} // break while loop if no intersections are found

          } //end while

  /*for(i = num_reflections - 1; i > -1; i--) {
	    obnum = objs_reflected[i];
	    
	    ARGB[0] = (1-reflectivity[obnum])*lrgb[obnum][0] + reflectivity[obnum]*ARGB[0];
	    ARGB[1] = (1-reflectivity[obnum])*lrgb[obnum][1] + reflectivity[obnum]*ARGB[1];
	    ARGB[2] = (1-reflectivity[obnum])*lrgb[obnum][2] + reflectivity[obnum]*ARGB[2];

	    } // end for i */
  
  if(num_reflections > 0) {
  i = objs_reflected[num_reflections-1];
...
int test01()
{
  ...
  light_in_eye_space[2] = 0;
	...
	check_intersect_ref(Rsource, Rtip, 0, x, y);
