...
int Refract(double irgb[3], double alpha, double s[3], 
  ...
  if (sinT2 > 1) {
    ...
    // You can use the refracted direction T to calculate the refracted color
    // and store it in argb

    // Example implementation (you'll need to replace this with your actual refracted color calculation)
    argb[0] = alpha * irgb[0];
    argb[1] = alpha * irgb[1];
    argb[2] = alpha * irgb[2];
  }
...
int Light_Model(double irgb[3], double alpha, double s[3], 
  ...
  if (alpha < 1.0) {
    return Refract(irgb, alpha, s, p, n, argb);
  } else {
		len = sqrt(n[0] * n[0] + n[1] * n[1] + n[2] * n[2]);
		if (len == 0) {
		  return 0;
...
int test01() {
  ...
  while (key != 'q') {
		...
		if (key == 's') {
			G_save_image_to_file("3d_RayReflect_v1.xwd") ;
			G_rgb(1,1,1) ; G_draw_string("Image saved as 3d_RayReflect_v1.xwd", 580,750) ;
			key = G_wait_key() ;
