...
int test01() {
  ...
  while (key != 'q') {
    ...
    bg_id = init_xwd_map_from_file("../XWDhelp/Change_xwd_styles/MistyForestJ.xwd");
    if (bg_id == -1) {
        printf("failure: can't init background map\n");
    }
    i = get_xwd_map_dimensions(bg_id, bg_dim);
    if (i == -1) {
        printf("failure: can't get background dimensions\n");
    }
    bg_width = bg_dim[0];
    bg_height = bg_dim[1];
		...
		for(x = -400; x < 400; x++) {
  	    ...
  	    if(num_intersections > 0) {
		          ...
		          ARGB[2] = ARGB[2] * transparency[obnum] + (1 - transparency[obnum]) * lrgb[obnum][2];
					...
					// No intersections, use background color
          double u = (x + 400.0) / 800.0;
          double v = (y + 400.0) / 800.0;

          int bg_x = (int)(u * bg_width);
          int bg_y = (int)(v * bg_height);

          double bg_color[3];
          i = get_xwd_map_color(bg_id, bg_x, bg_y, bg_color);
