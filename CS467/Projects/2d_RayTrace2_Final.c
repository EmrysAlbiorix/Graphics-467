...
int rayThing(double Rsource[], double Rtip[]) {
  ...
  double incoming[3] = {xBuff[0]-Rtip[0], xBuff[1]-Rtip[1], xBuff[2]-Rtip[2]};
  printf("Incoming: (%lf, %lf, %lf)\n", incoming[0], incoming[1], incoming[2]) ;
  
  normalizeVector(incoming,incoming,25);
  ...
  normalizeVector(reflection, reflection, 30) ;
...
int test01()
{
  ...
  double argb[3] ;
    ...
    while (G_wait_key() != 'q') ;
    G_save_image_to_file("2d_RayTrace2_Final.xwd") ;
}
