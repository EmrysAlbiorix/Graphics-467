...
// Does refraction
int refract(double I[3], double N[3], double eta, int isInside, double R[3]) {
  double n = eta;
  if (isInside) {
    n = 1.0 / n;
  }

  double cosI = dot_product(I, N);
  ...
  R[0] = I[0] * n - N[0] * (n * cosI - sqrt(k));
  R[1] = I[1] * n - N[1] * (n * cosI - sqrt(k));
  R[2] = I[2] * n - N[2] * (n * cosI - sqrt(k));
...
int test01() {
	...
	transparency[num_objects] = 0.5 ; // 50% transparency
  ...
  while (key != 'q') {
		...
		for(x = -400; x < 400; x++) {
			for(y = -400; y < 400; y++) {
				...
				while (insct == 1 && num_intersections < REF_DEPTH) {
		          ...
		          // Refraction
							double eta = 1.0 / refractive_index[OB];
							double I[3];
							...
							int isInside = dot_product(I, normal) > 0; // Check if the ray is inside the object

							double R[3];
							int refracted = refract(I, normal, eta, isInside, R); // Pass the isInside flag
		...
		if (key == 's') {
			G_save_image_to_file("3d_RayRefract_v2.xwd") ;
			G_rgb(1,1,1) ; G_draw_string("Image saved as 3d_RayRefract_v2.xwd", 580,750) ;
			key = G_wait_key() ;
