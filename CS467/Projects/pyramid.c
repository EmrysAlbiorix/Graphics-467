...
int main ()
{
  ...
  // Build bottom cross section from x axis to z axis with a cylinder.
  f = fopen("XYZ/cylinder.xyz","r") ;
  if (f == NULL) { printf("Can't open file %s\n", "cylinder.xyz") ;  exit(0) ;  }
  ...
  // Build left cross section from y axis to x axis with a cylinder.
  f = fopen("XYZ/cylinder.xyz","r") ;
  if (f == NULL) { printf("Can't open file %s\n", "cylinder.xyz") ;  exit(0) ;  }
  inherent_rgb[onum][0] = 0.5;
  inherent_rgb[onum][1] = 1.0;
  ...
  // Build right cross section from y axis to z axis with a cylinder.
  f = fopen("XYZ/cylinder.xyz","r") ;
  if (f == NULL) { printf("Can't open file %s\n", "cylinder.xyz") ;  exit(0) ;  }
  inherent_rgb[onum][0] = 0.5;
  inherent_rgb[onum][1] = 1.0;
  ...
  for (onum = 0 ; onum < numobjects ; onum++) {
    M3d_mat_mult_points (x[onum],y[onum],z[onum],  V, x[onum],y[onum],z[onum],numpoints[onum]) ;
  }
  ...
  //move everything back to WORLD SPACE for next increment of t.
  for (onum = 0 ; onum < numobjects ; onum++) {
    M3d_mat_mult_points (x[onum],y[onum],z[onum],  Vi, x[onum],y[onum],z[onum],numpoints[onum]) ;
  }
