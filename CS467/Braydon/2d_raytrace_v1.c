...
int test01()
{
  ...
  double argb[3] ;
    ...
    for (ytip = 200 ; ytip <= 600 ; ytip++) {
     ...
     //1st equation: (a) RsourceT[0]^2 + (b) 2*RsourceT[0]*(RtipT[0]-RsourceT[0]) + (c) (RtipT[0]-RsourceT[0])^2
     //2nd equation: (a) RsourceT[1]^2 + (b) 2*RsourceT[1]*(RtipT[1]-RsourceT[1]) + (c) (RtipT[1]-RsourceT[1])^2
     //A: (RtipT[0]-RsourceT[0])^2 + (RtipT[1]-RsourceT[1])^2
     //B: 2*RsourceT[0]*(RtipT[0]-RsourceT[0]) + 2*RsourceT[1]*(RtipT[1]-RsourceT[1]) 
     //C: [RsourceT[1]^2+RsourceT[0]^2-1]
     	
	double A,B,C;
    double t;
    double intersect[3];//x and y vector to hold intersect point (xt,yt)
	
	A =  (RtipT[0]-RsourceT[0])*(RtipT[0]-RsourceT[0]) + (RtipT[1]-RsourceT[1])*(RtipT[1]-RsourceT[1]);
	B = 2*RsourceT[0]*(RtipT[0]-RsourceT[0]) + 2*RsourceT[1]*(RtipT[1]-RsourceT[1]);
