...
int test01() {
	...
	light_in_eye_space[0] = 2 ;
	light_in_eye_space[1] = 3 ;
	...
	double proj_u2, proj_v2, u2, v2 ;
	int id, id2 ;
	int dim[2], dim2[2] ;
	double width, height, width2, height2 ;
	...
	id2 = init_xwd_map_from_file("waterrJ.xwd") ;
	if(id2 == -1) {
		printf("failure: can't init map\n") ;
	}
	i = get_xwd_map_dimensions(id2, dim2) ;
	if(i == -1) {
		printf("failure: can't dimensions\n") ;
	}
	width2 = dim2[0]; height2 = dim2[1] ;
		...
		for(y = -400; y < 400; y++) {
			...
			while (insct == 1 && num_intersections < REF_DEPTH) {
				...
				if(OB > -1) {
					...
					//texture mapping on main sphere
					if (OB == 1) {
					...
					// find unit normal in obj space
					i = find_normal(intersect, normal, obinv[OB], OB);
					// texture mapping on plate
					if (OB == 0) {
					//else {
					  u2 = 0.5 + (atan2(intersect[2], intersect[0]))/(2*M_PI);
				    v2 = 0.5 + asin(intersect[1])/M_PI;

				    proj_u2 = u2*width2;
					  proj_v2 = v2*height2;

					  i = get_xwd_map_color(id2, proj_u2, proj_v2, irgb[OB]);
					}
