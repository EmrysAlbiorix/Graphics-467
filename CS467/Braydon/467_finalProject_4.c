...
double EARTH[4][4],MOON[4][4],STARS[4][4];
double EARTHi[4][4],MOONi[4][4],STARSi[4][4];
double EARTHobj[4][4],MOONobj[4][4],STARSobj[4][4];
...
void plot (double (*f)(double u, double v), 
	...
	for (u = ulo; u <= uhi ; u+=0.001) {
    	for(v=vlo; v<= vhi; v+=0.001) {
   		point[0]=f(u,v);
   		point[1]=g(u,v);
   		point[2]=l(u,v);
		if(MOVEMENT==EARTHobj) {
			getColor(u,v,uhi,vhi,ulo,vlo,idEARTH);
		}  else if (MOVEMENT==MOONobj) {
			getColor(u,v,uhi,vhi,ulo,vlo,idMOON);
		...
		if(MOVEMENT==STARSobj) {SPECPOW=1000000000;} else {SPECPOW=50000000;}
...
int main() {
  ...
  light_in_eye_space[1] =  0.5;
  light_in_eye_space[2] = 5;
  ...
  SPECPOW = 30;
	...
	//movement sequence for Moon.
	Tn = 0; 
	Ttypelist[Tn] = SX ; Tvlist[Tn] =   .16; Tn++;
	Ttypelist[Tn] = SY ; Tvlist[Tn] =   .16; Tn++;
	Ttypelist[Tn] = SZ ; Tvlist[Tn] =   .16; Tn++;
	Ttypelist[Tn] = TZ ; Tvlist[Tn] =   2.3; Tn++;
	Ttypelist[Tn] = RY ; Tvlist[Tn] =  -60; Tn++;
	M3d_make_movement_sequence_matrix(MOON,MOONi,Tn,Ttypelist,Tvlist);
	...
	Ttypelist[Tn] = RY ; Tvlist[Tn] =   -36; Tn++;
  ...
  //Ttypelist[Tn] = RY ; Tvlist[Tn] =   26-0.07*258; Tn++;
	...
	idMOON = init_xwd_map_from_file ("SpaceTextures/moonB.xwd") ;// returns -1 on error, 1 if ok
  	if (idMOON == -1) { printf("failure\n") ;  exit(0) ; }
	
	idEARTH = init_xwd_map_from_file ("SpaceTextures/earthNightB.xwd") ;// returns -1 on error, 1 if ok
  	if (idEARTH == -1) { printf("failure\n") ;  exit(0) ; }
	
  	
  	idSTARS = init_xwd_map_from_file ("SpaceTextures/starsB.xwd") ;// returns -1 on error, 1 if ok
  	if (idSTARS == -1) { printf("failure\n") ;  exit(0) ; }
	...
	while (q < 1050) { //auto control: 250
		...
		M3d_mat_mult(MOON,rotate,MOON);
		
		Tn=0;
		Ttypelist[Tn] = RY ; Tvlist[Tn] =  -0.25; Tn++;
		M3d_make_movement_sequence_matrix(rotate,rotatei,Tn,Ttypelist,Tvlist);
	  ...
	  M3d_view (E, Ei,eye,coi,up);
		...
		//draw moon
		plot(sphere_x,sphere_y,sphere_z,-1*M_PI,M_PI,-1*M_PI/2,M_PI/2,MOONobj);
