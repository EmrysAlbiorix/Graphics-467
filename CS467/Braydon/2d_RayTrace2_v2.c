...
// Solves hyperbola
void hyper_solve(double a, double b, double c,double t1,double t2) {

//find both the t values for the hyperbola
...
// Finds the normal vector
//////////////////////////////////////////////////////////////
void getNormalHyperbola(int objnum, double normal[], double intersect[]) {
  // Moves back to obj-space
  double intersectObjSpace[3];
  M3d_mat_mult_pt(intersectObjSpace, obinv[objnum], intersect) ;
...
int rayThing(double Rsource[], double Rtip[]) {
  ...
  for (int objnum = 0; objnum < num_objects; objnum++) {
    ...
    // Find the intersection for each object
    double t=0,t1=0,t2=0;
    if (obtype[objnum] == 0) {
		t = -RsourceT[1]/(RtipT[1]-RsourceT[1]);
		double xT = RsourceT[0]+(RtipT[0]-RsourceT[0])*t;
		//printf("t: %lf xT: %lf\n",t,xT);
		if (t<0 || fabs(xT) > 1) {
			continue;
		}
    } else if(obtype[objnum] == 1) {
