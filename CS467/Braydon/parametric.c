...
double xv[1000],yv[1000],zv[1000];
int np;
...
void calc_normals() {
	...
	for(int i=0;i<np;i++) 
   	if (fmod(i+1,20)==0) {
   		...
   		normal[0]=20*normal[0]/mag;
   		normal[1]=20*normal[1]/mag;
   		G_line(x[i],y[i],normal[0]+x[i],normal[1]+y[i]);
   	}	
}
...
int main() {
	...
	M3d_mat_mult_points(xv,yv,zv,v,xv,yv,zv,np);
	...
	//---------------------square 1---------------------------------
	//initialize points for shape
	np=plot(square1_x,square1_y,0,4);
