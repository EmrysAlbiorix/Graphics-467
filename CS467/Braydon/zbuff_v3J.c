...
double REDi[4][4],YELLOWi[4][4],BLUEi[4][4];
...
int main() {
	...
	M3d_make_movement_sequence_matrix(RED,REDi,Tn,Ttypelist,Tvlist);
	
	
	//movement sequence for BLUE sphere.
	Tn = 0; 
  	Ttypelist[Tn] = TY ; Tvlist[Tn] =   0.5; Tn++;
	M3d_make_movement_sequence_matrix(BLUE,BLUEi,Tn,Ttypelist,Tvlist);

	
	//movement sequence for YELLOW sphere.
	Tn = 0; 
	Ttypelist[Tn] = SY ; Tvlist[Tn] =   0.5; Tn++;
  	Ttypelist[Tn] = TZ ; Tvlist[Tn] =   0.5; Tn++;
	M3d_make_movement_sequence_matrix(YELLOW,YELLOWi,Tn,Ttypelist,Tvlist);

	double RR[4][4],BB[4][4],YY[4][4] ;
	...
	while (q != 'q') {
		...
		M3d_mat_mult(RR,  E,RED) ;
	  	M3d_mat_mult(BB,  E,BLUE) ;
	  	M3d_mat_mult(YY,  E,YELLOW) ;
		...
		plot(sphere_x,sphere_y,sphere_z,-1*M_PI,M_PI,-1*M_PI/2,M_PI/2,RR);
		...
		inherent_rgb[1]=0;
		inherent_rgb[2]=1;
		plot(sphere_x,sphere_y,sphere_z,-1*M_PI,M_PI,-1*M_PI/2,M_PI/2,BB);
		
		//draw yellow sphere
		inherent_rgb[0]=1;
		inherent_rgb[1]=1;
		inherent_rgb[2]=0;
		plot(sphere_x,sphere_y,sphere_z,-1*M_PI,M_PI,-1*M_PI/2,M_PI/2,YY);
